...
class EcologyFunctional(PrototypeFunctional):
    ...
    def ecology(self, state):
        ...
        penalty_function = eco_domain*penalty_multiplier*((0.5*(dolfin.tanh(
            self.beta*(((U-Ua)**2)**0.5 - self.alpha))+1))**2)
        return penalty_function

    def Jt(self, state, turbine_field):
        eco_value = assemble(
            self.ecology(state)*self.problem.parameters.domain.dx)
        print "ecology = ", eco_value

        # the objective functional
        return -self.ecology(state)*self.problem.parameters.domain.dx
